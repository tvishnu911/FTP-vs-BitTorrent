...
public class Peer {
    ...
    public Peer(int peerID, String hostName, int port, int hasFile) throws Exception {
        ...
        bitFieldMap.put(peerID, new BitSet(CommonConfig.totalPieces));
        ...
        if (this.hasFile == 1 && (peerID == ParentThread.peerID)) {
            bitFieldMap.get(peerID).set(0, CommonConfig.totalPieces);
            numberOfCompleteDownloads = 1;
            fileData = FileProcessing.getDataInChunks(CommonConfig.fileSize, CommonConfig.pieceSize, CommonConfig.fileName);
                ...
                try {
                    ...
                    while (numberOfCompleteDownloads < ParentThread.peers.size()) {
                        ...
                        case Message.BITFIELD:
                            ...
                            if (message.payload.length * 8 >= CommonConfig.totalPieces) {
                                numberOfCompleteDownloads += 1;
                        ...
                        case Message.UNCHOKE:
                            ...
                            if (requestedPieces == null) {
                                requestedPieces = new BitSet(CommonConfig.totalPieces);
                            }
                        ...
                        case Message.REQUEST:
                            if (peerID == optimallyUnchokedPeer || preferredNeighbors.contains(peerID)) {
                                ...
                                byte[] piece = new byte[CommonConfig.pieceSize];
                        ...
                        case Message.PIECE:
                            ...
                            for (Peer peer : ParentThread.peers.values()) {
                                if (peer.socket != null) {
                                    ...
                                    if(fileData.size() == CommonConfig.totalPieces) {
                                        oStream.write(MessageManager.makeMessage(Message.HAVE, null, CommonConfig.totalPieces));
                        ...
                        case Message.HAVE:
                            ...
                            if(message.pieceIndex == CommonConfig.totalPieces) {
                                numberOfCompleteDownloads += 1;
                                fileWritten.put(peerID, true);
                                break;
                            }
                            ...
                            if (!(piecesRequired.length() == 0) && !(fileData.size() == CommonConfig.totalPieces)) {
                                outputStream.write(MessageManager.makeMessage(Message.INTERESTED, null, -1));
                                outputStream.flush();
                            } else {
                                outputStream.write(MessageManager.makeMessage(Message.NOT_INTERESTED, null, -1));
    ...
    public void writeFile() throws Exception {
            ...
            File file = new File("./" + ParentThread.peerID + "/" + CommonConfig.fileName);
            if (file.createNewFile()) {
                FileWriter fileWriter = new FileWriter("./" + ParentThread.peerID + "/" + CommonConfig.fileName, true);
